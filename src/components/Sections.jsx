...
import { SectionHead, Tags, TextureInverted, Beacon } from './Chrome';
...
// Served from public/, so Vite copies it to the site root untouched.
const RESUME_URL = '/Varun-N-Resume.pdf';
...
export function Hero() {
  ...
  return (
    <header id="top" className="wrap scene" style={{ position: 'relative', paddingTop: 96, overflow: 'hidden' }}>
      ...
      <div className="rail asm" style={{ '--d': '300ms' }} aria-hidden="true">
        <span className="m">Software engineer &middot; Chennai</span>
        <span className="rail-line" />
        <span className="m rail-year">2026</span>
      </div>

      {/* v20's meta row above the headline */}
      <div className="asm metarow" style={{ '--d': '150ms' }}>
        <span className="m" style={{ color: 'var(--accent)' }}>Portfolio</span>
        <span className="m" style={{ marginLeft: 'auto', color: 'var(--muted)' }}>Surfboard Payments &middot; since 2022</span>
      </div>

      <div className="g-hero preserve" style={{ display: 'grid', gridTemplateColumns: '1.6fr minmax(238px, .4fr)', gap: 56, alignItems: 'start' }}>
        <div className="par-y" data-py="14">
          <h1 style={{ position: 'relative' }}>
            <NameMark text="Varun N." ariaLabel="Varun N." tilt={5} />
          </h1>
          {/* a standfirst under the name, so the column reads as composed
              rather than as a headline floating in empty space */}
          <p className="asm" style={{ '--d': '820ms', marginTop: 26, maxWidth: '30ch', fontSize: 'clamp(16px,1.5vw,20px)', lineHeight: 1.5, color: 'var(--muted)', textWrap: 'pretty' }}>
            Payments infrastructure &mdash; terminal apps, acquirer integrations, and the
            services that keep 35,000 devices online.
          </p>
          <div className="asm scrollcue m" style={{ '--d': '1020ms' }} aria-hidden="true">
            <span>Scroll</span><span className="scrollcue-line" />
          </div>
        </div>

        <div className="par-y" data-py="40">
        <div className="asm card" ref={specRef} style={{ '--d': '420ms', paddingTop: 10 }}>
      ...
      <div className="asm live-rule g-about" style={{ position: 'relative', display: 'grid', gridTemplateColumns: '.14fr .52fr .34fr', gap: 48, marginTop: 64, paddingTop: 34, borderTop: '1px solid var(--fg)' }}>
        ...
        <p style={{ fontSize: 'clamp(20px,2.3vw,31px)', lineHeight: 1.24, letterSpacing: '-.02em', textWrap: 'pretty' }}>
          I build the software that turns a tap into an authorization &mdash; terminal apps, acquirer integrations, and the fleet services behind them.
        </p>
        <div>
          <p style={{ fontStyle: 'italic', color: 'var(--accent)', fontSize: 16, lineHeight: 1.5, textWrap: 'pretty' }}>
            Four years in one authorization path &mdash; deep enough to know where it breaks.
          </p>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 24 }}>
            <a href="#work" ref={ctaRef} className="m lift cta-solid">
              <span className="cta-label">View selected work</span>
              <DotArrow />
            </a>
            <a href="#contact" className="m lift" style={{ border: '1px dashed var(--rule)', padding: '14px 22px' }}>Contact</a>
            <a href={RESUME_URL} download className="m lift" style={{ border: '1px dashed var(--rule)', padding: '14px 22px' }}>Download résumé</a>
          </div>
        </div>
      </div>
    </header>
...
export function Marquee() {
  ...
  return (
    <div className="marquee scene asm" style={{ '--d': '120ms', marginTop: 72, borderTop: '1px dashed var(--rule)', borderBottom: '1px dashed var(--rule)', overflow: 'hidden', padding: '22px 0' }}>
...
function WorkRow({ item, first }) {
  return (
    <article className={`asm row g-row z-drift cells${first ? ' live-rule' : ''}`} data-drift="22"
...
export function Work() {
  return (
    <section id="work" className="wrap scene-deep" style={{ position: 'relative', paddingTop: 104, scrollMarginTop: 70, overflow: 'hidden' }}>
      <SectionHead index="01 &mdash; Work" title="Systems I've" em="shipped" aside="5 selected &middot; 12 owned" />
      <div className="preserve">
        {WORK.map((item, i) => <WorkRow key={item.id} item={item} first={i === 0} />)}
      </div>
      <div className="asm live-rule" style={{ borderTop: '1px solid var(--fg)', borderBottom: '1px solid var(--fg)', marginTop: 8, padding: '26px 0', display: 'flex', alignItems: 'baseline', gap: 24, flexWrap: 'wrap' }}>
        ...
        <div style={{ fontSize: 15.5, color: 'var(--muted)', flex: 1, minWidth: '18rem', textWrap: 'pretty' }}>
          fin-key scheduling &middot; shared terminal libraries &middot; Surf-Buggies signing worker &middot; developer &amp; partner APIs &middot; service consolidation across ~87 services
        </div>
      </div>
    </section>
  );
...
export function Experience() {
  ...
  return (
    <section id="experience" className="wrap scene" style={{ paddingTop: 104, scrollMarginTop: 70 }}>
      <SectionHead index="02 &mdash; Experience" title="Where I've" em="worked" aside="1 company" />
      <div className="asm cell card" ref={ref} style={{ marginTop: 46, border: '1px dashed var(--rule)', padding: '34px 38px 38px' }}>
        ...
        <div data-layer="1.3">
          <div className="m" style={{ color: 'var(--muted)' }}>Feb 2022 &mdash; present &middot; 4+ years</div>
          <h3 style={{ fontSize: 'clamp(26px,3vw,40px)', marginTop: 14 }}>Surfboard Payments</h3>
          <div className="m" style={{ color: 'var(--muted)', marginTop: 8 }}>Software Engineer &middot; Chennai, India</div>
        </div>
        <p data-layer="0.8" style={{ fontSize: 16.5, lineHeight: 1.62, marginTop: 22, maxWidth: '80ch', textWrap: 'pretty' }}>
          Joined as an intern and stayed. I work across the authorization path &mdash; terminal software, acquirer integrations, card cryptography and the fleet services that keep 25,000 live devices configured and reachable.
        </p>
...
function CapCard({ cap, i }) {
  ...
  return (
    <div ref={ref} className="asm card cap-plate" style={{ '--d': `${i * 70}ms`, borderTop: '1px solid var(--fg)', padding: '16px 0 4px' }}>
      <div data-layer="1.4" style={{ display: 'flex', alignItems: 'baseline', gap: 11 }}>
        <span className="cellnum">{cap.n}</span>
        <span className="m" style={{ color: 'var(--muted)' }}>{cap.area}</span>
      </div>
      <h3 data-layer="1" style={{ fontSize: 19, marginTop: 12, lineHeight: 1.18 }}>{cap.title}</h3>
      <ul className="cap-list" data-layer="0.5">
        {cap.items.map((it, k) => <li key={it} className="cap-item" style={{ '--k': k }}>{it}</li>)}
      </ul>
    </div>
  );
}
...
export function Capabilities() {
  return (
    <section id="capabilities" className="wrap scene-deep" style={{ position: 'relative', paddingTop: 104, scrollMarginTop: 70, overflow: 'hidden' }}>
      <SectionHead index="03 &mdash; Capabilities" title="What I build" em="with" aside="5 areas" />
      <div className="g-cap preserve cells" style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 34, marginTop: 46 }}>
...
function ProjectCard({ p, i }) {
  ...
  return (
    <div ref={ref} className={`asm card cell${i === 0 ? ' halftone' : ''}`}
      style={{ '--d': `${i * 110}ms`, borderRight: '1px dashed var(--rule)', borderBottom: '1px dashed var(--rule)', padding: '32px 34px 34px' }}>
  ...
  return (
    <section id="independent" className="wrap scene-deep" style={{ position: 'relative', paddingTop: 104, scrollMarginTop: 70, overflow: 'hidden' }}>
      <div className="asm" style={{ position: 'relative', display: 'flex', alignItems: 'baseline', gap: 34, flexWrap: 'wrap' }}>
        <span className="m" style={{ color: 'var(--muted)', minWidth: 170 }}>04 &mdash; Independent</span>
      ...
      <div className="g-3 preserve" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', marginTop: 46, borderTop: '1px solid var(--fg)' }}>
...
export function Approach() {
  return (
    <section id="approach" className="wrap scene" style={{ position: 'relative', paddingTop: 104, scrollMarginTop: 70, overflow: 'hidden' }}>
      <div className="asm g-about" style={{ position: 'relative', display: 'grid', gridTemplateColumns: '.14fr .52fr .34fr', gap: 48 }}>
        <div className="m" style={{ color: 'var(--muted)' }}>05 &mdash; Approach</div>
      ...
      <div className="live-rule g-cap preserve" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', marginTop: 56, borderTop: '1px solid var(--fg)' }}>
        {APPROACH.map((t, i) => (
          <div key={t} className={`asm card${i === 0 ? ' halftone' : ''}`} style={{ '--d': `${i * 80}ms`, borderRight: '1px dashed var(--rule)', borderBottom: '1px dashed var(--rule)', padding: '28px 26px 34px' }}>
            <div className="cellnum" data-layer="1.3">{String(i + 1).padStart(2, '0')}</div>
...
const FOOT_COLS = [
  ...
  ['Quick links', [['Work', 'selected', '#work'], ['Capabilities', 'stack', '#capabilities'], ['Résumé', 'pdf', RESUME_URL]]],
];
...
export function Contact() {
  ...
  return (
    <div id="contact" style={{ marginTop: 120, background: 'var(--inv-bg)', color: 'var(--inv-fg)', scrollMarginTop: 70, position: 'relative', overflow: 'hidden', borderTop: '1px solid var(--seam)' }}>
      <PaperSurface scoped prefix="inv-paper" />
      <TextureInverted />
      <HalftoneBackdrop />
      <SignatureMark />
      <div className="wrap" style={{ paddingTop: 96, paddingBottom: 40, position: 'relative', zIndex: 1 }}>
        <div className="g-foot" style={{ display: 'grid', gridTemplateColumns: '1.1fr .6fr .6fr .6fr', gap: 56 }}>
          <div>
            <div className="asm m" style={{ '--d': '0ms', color: 'var(--inv-muted)' }}>06 &mdash; Contact</div>
            <h2 className="asm" style={{ '--d': '90ms', fontSize: 'clamp(38px,5.4vw,86px)', letterSpacing: '-.048em', lineHeight: .94, marginTop: 22, fontWeight: 800 }}>
              Building something?<br /><em style={{ fontStyle: 'italic', fontWeight: 500, color: 'var(--inv-accent)' }}>Let's discuss.</em>
            </h2>
            {/* the assemble drives the wrapper's transform and the magnetic
                hover drives the link's, so the two never fight over it */}
            <div className="asm" style={{ '--d': '300ms', marginTop: 30 }}>
              <a href={`mailto:${EMAIL}`} ref={mailRef} className="m lift"
                style={{ display: 'inline-flex', alignItems: 'center', gap: 18, border: '1px dashed var(--inv-rule)', borderRadius: 100, padding: '13px 18px 13px 24px', color: 'var(--inv-fg)' }}>
                {EMAIL}<span className="pix" aria-hidden="true"><i>&rarr;</i><b>&rarr;</b></span>
              </a>
            </div>
          </div>
          {FOOT_COLS.map(([label, links], i) => (
            <div key={label} className="asm" style={{ '--d': `${160 + i * 85}ms` }}>
        ...
        <div className="g-foot" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 34, marginTop: 88, paddingTop: 22, borderTop: '1px dashed var(--inv-rule)' }}>
          <div className="asm m" style={{ '--d': '430ms', color: 'var(--inv-muted)' }}>Chennai, India &middot; UTC+5:30</div>
          <div className="asm m" style={{ '--d': '500ms' }}><Beacon inverted>Open to work</Beacon></div>
          <div className="asm m" style={{ '--d': '570ms', color: 'var(--inv-muted)', textAlign: 'right' }}>© 2026 Varun N</div>
        </div>
      </div>
    </div>
  );
